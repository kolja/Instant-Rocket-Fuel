class Map
    constructor: (opts) ->
        ...
        # in opts["pattern"] you can either pass a string like "simple", "square" or "cross"
        # in which case the respective method will be called. Alternatively, you can pass your own custom function.
        if typeof opts["pattern"] is "function"
            @read = opts["pattern"]
        else
            switch opts["pattern"]
                when "simple"
    ...
    loadMapDataFromImage: ->
        $(@map).load =>
            ...
            for p,i in data by 4
                ...
                @mapData[row] ?= []
                # why .toHex() ??? why are you storing strings?? just leave it as number and dont worry about it
                @mapData[row].push [Number(data[i]).toHex(), Number(data[i + 1]).toHex(), Number(data[i + 2]).toHex(),
                                    Number(data[i + 3]).toHex()]
...
class Tile
    ...
    isWalkable: ->
        @green is 0 # ??? isn't this game logic??
